    ...
    */
    ...
    // Desplegable "Estado actual del paciente" (col 47): Operado | En lista de espera | Referido | Suspendido.
    ...
    return 'NO APTO'; // pendiente (aún sin evaluar) o negado
    }
    ...
    /** Formatea la columna "celular" como texto plano y centrada (corrige la
        alineación inconsistente que queda al mezclar escritura manual y script).
        Ejecutar manualmente una vez: formatearCelular → Ejecutar. */
    ...
    sheet.getRange(headerRow + 1, col, n, 1)
        .setNumberFormat('@')
        .setHorizontalAlignment('center');
    }

    /** Devuelve el siguiente "ID registro" y el total, igual que lo calcula upsert().
        (Opcional: la app puede consultarlo para sugerir el ID exacto.) */
    function doGet(e) {
    var ss = SpreadsheetApp.openById(SS_ID);
    var sheet = ss.getSheetByName(SHEET_NAME);
    ...
    var p = {
        ...
        establecimiento_destino: 'HOSPITAL DISTRITAL DE LAREDO', codigo_destino: '5231',
        establecimiento_origen: 'HOSPITAL DISTRITAL DE LAREDO', codigo_origen: '5231',
        ...
        orden_intervencion: '01', laboratorio_completo: true, ekg: true,
        fecha_examen1: '2026-08-14', fecha_examen2: '2026-08-14'
    };
    var row = upsert(p);
